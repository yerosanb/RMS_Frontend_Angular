...
@Component({
  selector: 'app-view-currency-request',
  templateUrl: './view-currency-request.component.html',
  styleUrls: ['./view-currency-request.component.css'],
})
export class ViewCurrencyRequestComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  // getStatus(){
  //   this.service.getStatus(this.id).subscribe((data)=>{
  //     console.log(data);
  //     this.status = data;
  //     console.log(data);
  //   })
  ...
  ngOnInit(): void {
    ...
    this.dtOption1 = {
      ...
      ajax: (dataTablesParameters: any, callback: any) => {
        this.service.fetchAllCurrency().subscribe(
            async (resp: any) => {
                        if (resp != null) {
                          console.log(
                            'response for table: ' +
        ...
        // },
          ...
          render: function (data: any, type: any, full: any) {
            ...
            return (
              ...
              // '"  -role="' +
              // full.id +
              // '<div class="shadow p-1 mb-2 bg-light rounded *ngIf="status1" style="color:green">Pending</div>'
            );
          },
        },
      ],
    ...
    this.dtOption2 = {
      ...
      // select: true,
      ajax: (dataTablesParameters: any, callback: any) => {
        this.service.fetchApprovedCurrency().subscribe(
         async (resp: any) => {
            if (resp != null) {
              console.log(
        ...
        },
        {
          ...
          render: function (data: any, type: any, full: any) {
            document
              ...
              ?.classList.remove('dt-button');
            return (
              // '<button class="btn btn-primary" edit-currency-id="' +
              // full.id +
              // '" style="margin-right:4px"> Edit</button>'+

              // '<button class="btn btn-danger" delete-currency-id="' +
              // full.id +
              // '">Delete</button>'

              '<button class="btn btn-outline-primary" edit-currency-id="' +
              full.id +
    ...
    this.dtOption4 = {
      ...
      // select: true,
      ajax: (dataTablesParameters: any, callback: any) => {
        this.service.fetchRejectedCurrency().subscribe(
              async (resp: any) => {
            if (resp != null) {
              console.log(
                'response for table: ' + JSON.stringify(resp, null, 2)
