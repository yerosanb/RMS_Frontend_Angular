...
@Component({
  selector: 'app-payable-matched',
  templateUrl: './payable-matched.component.html',
  styleUrls: ['./payable-matched.component.css']
})
export class PayableMatchedComponent {
  ...
  ngOnInit(): void {
    ...
    try {
      this.dtOptions_payable_credit = {
        ...
        ajax: (dataTablesParameters: any, callback: any) => {
          this.reconViewService.getPayableCreditForView(formatDate(this.initialDateValue, 'yyyy-MM-dd', 'en-US')).subscribe(
            async (resp: any) => {
              if (resp != null) {
                console.log(
    ...
    try {
      this.dtOptions_payable_debit = {
        ...
        // ajax: '../../../../assets/data/data.json',
        ajax: (dataTablesParameters: any, callback: any) => {
          this.reconViewService.getPayableDebitForView(formatDate(this.initialDateValue, 'yyyy-MM-dd', 'en-US')).subscribe(
            async (resp: any) => {
              if (resp != null) {
                console.log(
                  'response for table: ' + JSON.stringify(resp, null, 2)
  ...
  UnmatchTransactions() {
    ...
    var selection_true: boolean = false;
    this.dtElements.forEach((dtElement: DataTableDirective, index: number) => {
      if (index == 0) {
        dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
