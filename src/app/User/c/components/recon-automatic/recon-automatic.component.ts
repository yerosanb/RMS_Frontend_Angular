...
@Component({
  selector: 'app-recon-automatic',
  templateUrl: './recon-automatic.component.html',
  styleUrls: ['./recon-automatic.component.css'],
})
export class ReconAutomaticComponent implements OnInit {
  ...
  ats_tenth_column_title = 'Core Id';
  ...
  onReconDateChange(value: Date): void {
    ...
    this.localStorageService.store(
      'recon_current_date_rtgs_automatic',
  ...
  ngOnInit(): void {
    if (
      this.localStorageService.retrieve('recon_current_date_rtgs_automatic') ==
      null
    ) {
      ...
      if (index == 0) {
        ...
        dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
          ...
          if (this.selections > 0) {
            ...
            if (data_1_id.length != 1 || data_2_id.length !=1 ) {
              this.toast.error({
                detail: 'ERROR',
                summary: 'Please select 1 transaction both ATS and CORE side to Match.',
              });
            }
                      ...
                      this.dtElements.forEach(
                        (dtElement: DataTableDirective, index: number) => {
                          ...
                          if (index == 0) {
                            ...
                            $('#rtgs_core_table').DataTable().ajax.reload()
                          }
                          // else {
                          //   // dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
