...
@Component({
  selector: 'app-account-remark-replay',
  templateUrl: './account-remark-replay.component.html',
  styleUrls: ['./account-remark-replay.component.css']
})
export class AccountRemarkReplayComponent{
  //Swal Config
  ...
  submitremarkReplay() {
    ...
    if (this.submitRemarkForm.valid) {
      ...
      this.AddAccountRemark(this.remark);
  ...
  AddAccountRemark(replayRemarkPayload: ReplayRemarkPayload) {

    this.remarkService.postAccountRemark(replayRemarkPayload).subscribe(data => {
      console.log(data);
      Swal.fire({
        icon: 'success',
        title: 'Success',
      ...
      this.router.navigate(['user/view-account-remark']);
