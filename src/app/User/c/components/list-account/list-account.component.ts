  ...
  ngOnInit(): void {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback: any) => {
        this.service.getAccountList().subscribe((resp) => {
          console.log('account ajax');
        ...
        },
        {
          ...
          render: function (data: any, type: any, full: any) {
            ...
            return (
              ...
              // '<button class="btn btn-outline-danger" delete-account-id="' +
              // full.id +
    ...
    this.renderer.listen('document', 'click', (event) => {
      if (event.target.hasAttribute('edit-id')) {
        this.router.navigate([
          'user/list-account/edit-account/' +
          event.target.getAttribute('edit-id'),
        ]);
      } else if (event.target.hasAttribute('delete-Account-id')) {
        this.service
          .deleteAccount(event.target.getAttribute('delete-Account-id'))
          .subscribe(
            (data) => {
              Swal.fire({
                icon: 'success',
                  ...
                  this.utilService.refreshToken().subscribe(
                    (data: any) => {
                      if (data === true) {
                        ...
                        this.service
                          .deleteAccount(
                            event.target.getAttribute('delete-Account-id')
                          )
                          .subscribe(
                            (data) => {
                              Swal.fire({
