...
@Component({
  selector: 'app-view-rtgs-matched',
  templateUrl: './view-rtgs-matched.component.html',
  styleUrls: ['./view-rtgs-matched.component.css']
})
export class ViewRtgsMatchedComponent {
  ...
  ngOnInit(): void {
      ...
      this.dtOptions_ats = {
        ...
        ajax: (dataTablesParameters: any, callback: any) => {
          this.reconViewService
            .getRtgsAtsForView(formatDate(this.initialDateValue, 'yyyy-MM-dd', 'en-US'))
            .subscribe(
              async (resp: any) => {
                console.log(
                    ...
                    this.utilService.refreshToken().subscribe(
                      (data) => {
                        if (data === true) {
                          ...
                          //================================================================================
                          this.reconViewService.getRtgsAtsForView
                          (
                              formatDate(
                                this.initialDateValue,
                                'yyyy-MM-dd',
    ...
    try {
      this.dtOptions_core = {
        ...
        // ajax: '../../../../assets/data/data.json',
        ajax: (dataTablesParameters: any, callback: any) => {
          this.reconViewService.getRtgsCoreForView(formatDate(this.initialDateValue, 'yyyy-MM-dd', 'en-US')).subscribe(
            async (resp: any) => {
              if (resp != null) {
                console.log(
                  'response for table: ' + JSON.stringify(resp, null, 2)
