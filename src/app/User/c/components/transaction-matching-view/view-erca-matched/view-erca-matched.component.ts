...
@Component({
  selector: 'app-view-erca-matched',
  templateUrl: './view-erca-matched.component.html',
  styleUrls: ['./view-erca-matched.component.css'],
})
export class ViewErcaMatchedComponent {
          ...
          buttons: [
            ...
            ,
            {
              ...
              action: function (e: any, dt: any, node: any, config: any) {
                ...
                var data_1_id: number[] = [];
                for (let i = 0; i < rows.length; i++) {
                  // formData.append('id', rows[i].id)
                  console.log('row having database ID' + rows[i].id);
                  data_1_id.push(rows[i].id);
                  // that.reconViewService.deleteReconItem(rows[i].id);
                }
                that.reconViewService
                  .deleteReconItem(data_1_id)
                  .subscribe((data) => {
                    Swal.fire({
                      icon: 'success',
                      title: 'Success',
                ...
                console.log('row having database ID' + data_1_id);

                alert(
                  'There are ' + rows.length + ' selected rows in the table'
                );
              },
            },
        ...
        buttons: {
          buttons: [
            ...
            // },
            {
              ...
              text: 'Delete',
              action: function (e: any, dt: any, node: any, config: any) {
                dt.on('delete-recon-items', function () {
                  var rows = dt.rows({ selected: true }).data().toArray();
                  alert(
                    'There are ' + rows.length + ' selected rows in the table'
                  );
                  for (let i = 0; i < rows.length; i++) {
                    ...
                    var c = rows[i].id;
                    //  var  that= this;
                    //that.reconViewService.deleteReconItem(c);
                  }
                });
              },
            },
