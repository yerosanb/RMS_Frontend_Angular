...
@Component({
  selector: 'app-issue-matched',
  templateUrl: './issue-matched.component.html',
  styleUrls: ['./issue-matched.component.css']
})
export class IssueMatchedComponent implements OnInit {
  ...
  onMinMatchDateChange(value: Date): void {
    ...
    this.localStorageService.store('min_matched_current_date_issue', this.initialMinDateValue);
    if (!this.reload_checker)
      this.dtElements.forEach(
        (dtElement: DataTableDirective, index: number) => {
  ...
  onMaxMatchDateChange(value: Date): void {
    ...
    this.localStorageService.store('max_matched_current_date_issue', this.initialMaxDateValue);
    if (!this.reload_checker)
      this.dtElements.forEach(
        (dtElement: DataTableDirective, index: number) => {
  ...
  ngOnInit() {
    if (this.localStorageService.retrieve('min_matched_current_date_issue') == null) {
      this.initialMinDateValue = new Date();
      this.localStorageService.store(
        'min_matched_current_date_issue',
        this.initialMinDateValue
      );
    } else
      this.initialMinDateValue = new Date(
        this.localStorageService.retrieve('min_matched_current_date_issue')
      );
    if (this.localStorageService.retrieve('max_matched_current_date_issue') == null) {
      this.initialMaxDateValue = new Date();
      this.localStorageService.store(
        'max_matched_current_date_issue',
        this.initialMaxDateValue
      );
    } else
      this.initialMaxDateValue = new Date(
        this.localStorageService.retrieve('max_matched_current_date_issue')
      );
    this.dtOptions_core = {
      serverSide: false,
      scrollX: true,
      ...
      ajax: (dataTablesParameters: any, callback: any) => {
        this.issueAccountService.getCoreIssueForeViewMatched(
          formatDate(this.initialMinDateValue, 'yyyy-MM-dd', 'en-US').toString(),
    ...
    this.dtOptions_qbs = {
      ...
      ajax: (dataTablesParameters: any, callback: any) => {
        ...
        this.issueAccountService.getQbsIssueForeViewMatched(
          formatDate(this.initialMinDateValue, 'yyyy-MM-dd', 'en-US').toString(),
          formatDate(this.initialMaxDateValue, 'yyyy-MM-dd', 'en-US').toString()
        ).subscribe(
          async (resp: any) => {
            ...
            if (match_id_ats == match_id_core) {
              ...
              Swal.fire({
                ...
                if (result.isConfirmed) {
                  ...
                  this.issueAccountService
                    .unmatchIssueTransactions(data_1_id.toString(), data_2_id.toString())
                    .subscribe(
                      (data: any) => {
                        if (data == true) {
