...
@Component({
  selector: 'app-issue-core-reversal-matched',
  templateUrl: './issue-core-reversal-matched.component.html',
  styleUrls: ['./issue-core-reversal-matched.component.css']
})
export class IssueCoreReversalMatchedComponent implements OnInit {
  ...
  onMinMatchDateChange(value: Date): void {
    ...
    this.localStorageService.store('min_matched_current_date_issue_core_reversal', this.initialMinDateValue);
    if (!this.reload_checker)
      this.dtElements.forEach(
        (dtElement: DataTableDirective, index: number) => {
  ...
  onMaxMatchDateChange(value: Date): void {
    ...
    this.localStorageService.store('_mamaxtched_current_date_issue_core_reversal', this.initialMaxDateValue);
    if (!this.reload_checker)
      this.dtElements.forEach(
        (dtElement: DataTableDirective, index: number) => {
  ...
  ngOnInit() {
    if (this.localStorageService.retrieve('min_matched_current_date_issue_core_reversal') == null) {
      this.initialMinDateValue = new Date();
      this.localStorageService.store(
        'min_matched_current_date_issue_core_reversal',
        this.initialMinDateValue
      );
    } else
      this.initialMinDateValue = new Date(
        this.localStorageService.retrieve('min_matched_current_date_issue_core_reversal')
      );
    if (this.localStorageService.retrieve('max_matched_current_date_issue_core_reversal') == null) {
      this.initialMaxDateValue = new Date();
      this.localStorageService.store(
        'max_matched_current_date_issue_core_reversal',
        this.initialMaxDateValue
      );
    } else
      this.initialMaxDateValue = new Date(
        this.localStorageService.retrieve('max_matched_current_date_issue_core_reversal')
      );
    this.dtOptions_core = {
      serverSide: false,
      scrollX: true,
      ...
      ajax: (dataTablesParameters: any, callback: any) => {
        this.issueAccountService.getCoreIssueForeViewreversal(formatDate(this.initialMinDateValue, 'yyyy-MM-dd', 'en-US').toString(),
                    ...
                    (data) => {
                      if (data === true) {
                        ...
                        this.issueAccountService
                          .getCoreIssueForeViewreversal(formatDate(this.initialMinDateValue, 'yyyy-MM-dd', 'en-US').toString(),
            ...
            if (rowData[0].match_id == rowData[1].match_id) {
              Swal.fire({
                ...
                if (result.isConfirmed) {
                  ...
                  this.issueAccountService
                    .unmatchIssueCoreReversalTransactions(data_1_id.toString())
                    .subscribe(
                      (data: any) => {
                        if (data == true) {
