...
@Component({
  selector: 'app-receivable-automatic',
  templateUrl: './receivable-automatic.component.html',
  styleUrls: ['./receivable-automatic.component.css']
})
export class ReceivableAutomaticComponent implements OnInit {
  ...
  core_eighth_column_title = ' Debit Id';
  ...
  ngOnInit(): void {
        ...
        this.dtOptions_credit = {
          ...
          ajax: (dataTablesParameters: any, callback: any) => {
            this.receivableService
              .getReceivableCreditForReconAuto()
              .subscribe(
                async (resp: any) => {
                  if (resp != null) {
                    console.log(
    ...
    try {
      this.dtOptions_debit = {
        ...
        ajax: (dataTablesParameters: any, callback: any) => {
          this.receivableService
            .getReceivableDebitForReconAuto()
            .subscribe(
              async (resp: any) => {
                if (resp != null) {
                  console.log(
      ...
      if (index == 0) {
        dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
          dtInstance.on('select', function (e, dt, type, indexes) {
            ...
            that.amountAll = rowData[0].amount;
            that.core_input_1 = that.refAll;
            that.core_input_2 = that.amountAll;
        ...
        if (index == 0) {
          dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
            ...
            if (rowData.length > 0) {
              ...
              for (let i = 0; i < rowData.length; i++) {
                ...
                data_1_description.push(rowData[i].additional_information)
                data_1_type.push(rowData[i].value_date_type);
                data_1_reference = rowData[i].reference;
          ...
          dtElement.dtInstance.then((dtInstance: DataTables.Api) => {
            ...
            for (let i = 0; i < rowData.length; i++) {
              ...
              data_2_description.push(rowData[i].additional_information)
