...
@Component({
  selector: 'app-view-mms-stock-deleted',
  templateUrl: './view-mms-stock-deleted.component.html',
  styleUrls: ['./view-mms-stock-deleted.component.css']
})
export class ViewMmsStockDeletedComponent implements OnInit, AfterViewInit {
  //Swal Config
  ...
  core_third_column_title = 'Account Segment';
  core_fourth_column_title = 'Amount';
  core_fifth_column_title = 'Main_pg';
  // core_sixth_column_title = 'Credit';
  core_seventh_column_title = 'DR_CR';
  core_eighth_column_title = 'Transaction Date';
  core_ninth_column_title = 'Store Name';
  core_tenth_column_title = 'Category Description';
  ...
  constructor(
    ...
    private reconService: ReconManualFixedAssetService,
  ...
  ngOnInit(): void {
    ...
    this.detailModal = new window.bootstrap.Modal(
      document.getElementById('details_transaction_modal_stock_mms')
    );
    this.dtOptions = {
      serverSide: false,
      scrollX: true,
      ...
      ajax: (dataTablesParameters: any, callback: any) => {
        this.stockService.getDeleteStockMMSTransaction().subscribe(
          async (resp: any) => {
            if (resp != null) {
              console.log(
          ...
          ,
          {
            ...
            action: function (e: any, dt: any, node: any, config: any) {
              ...
              for (let i = 0; i < rows.length; i++) {
                that.idd = rows[i].id;
                $('#view_detail_mms').DataTable().ajax.reload()
                that.openModal(dt);
              }
            },
          },
    ...
    this.dtOptions_detail = {
      ...
      ajax: (dataTablesParameters: any, callback: any) => {
        this.stockService.getEditedDetailStockMMSTransaction(that.idd).subscribe(
          async (resp: any) => {
                ...
                this.utilService.refreshToken().subscribe(
                  (data) => {
                    if (data === true) {
                      ...
                      //================================================================================
                      this.stockService
                        .getEditedDetailStockMMSTransaction(that.idd).subscribe(
                          async (resp: any) => {
                            if (resp != null) {
                              console.log(
                                'response for table: ' +
                              ...
                              for (let i = 0; i < this.length; i++) {
                                ...
                                if (resp[i]?.edit_reason_id == resp[i + 1]?.edit_reason_id) {
                                  ...
                                  aa.reason = resp[i]?.reason
                                  aa.by = resp[i]?.firstname + ' ' + resp[i]?.lastname
                                  aa.date = resp[i]?.datedetails_transaction_modal
                                  this.reasonEditDelete.push(aa)
                                  i++
                                }
                                else {
