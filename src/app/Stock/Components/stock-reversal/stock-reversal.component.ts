...
export class StockReversalComponent {
    ...
    try {
      this.dtOptions_core_stock = {
        ...
        ajax: (dataTablesParameters: any, callback: any) => {
          this.stockService.getStockCoreReversal(formatDate(this.initialDateValue, 'yyyy-MM-dd', 'en-US')).subscribe(
            async (resp: any) => {
              if (resp != null) {
                console.log(
            ...
            if (data_2_id .length==2) {
              ...
              Swal.fire({
                ...
                if (result.isConfirmed) {
                  ...
                  this.stockService
                    .unmatchReversalTransactions(data_2_id.toString())
                    .subscribe(
                      (data: any) => {
                        if (data == true) {
  ...
  showMisSelection() {
    this.toast.error({ detail: 'ERROR', summary: 'Please select All  Filtered  transaction for  UnMatch.' });
  }
