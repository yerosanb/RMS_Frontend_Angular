...
import { UserActivitiesComponent } from 'src/app/Admin/c/user-activities/user-activities.component';
...
export class CurrencyRequestsComponent implements OnInit, AfterViewInit {
  ...
  seventh_column_title = 'Sent_from';
  eight_column_title = 'Actions';
  ...
  ngOnInit(): void {
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback: any) => {
        ...
        //     {}
        //   )
        this.approverService.getAllCurrencyRequests().subscribe(
          async (resp: any) => {
            if (resp != null) {
              console.log(
          ...
          render: function (data: any, type: any, full: any) {
            ...
            if (full.request_status == 0)
              ...
              '"><i class="mdi mdi-check"></i>Approve</button>'+" "+
                ...
                '<button class="btn btn-outline-warning btn-rounded" send-currency-remark="' +
                full.id +
  ...
  saveRemark(request_id:any,remark: Remark) {
    ...
    this.approverService.currencyRemark(request_id,remark).subscribe(
      async (data) => {
        if (data == true) {
  ...
  ngAfterViewInit(): void {
    this.renderer.listen('document', 'click', (event) => {
      if (event.target.hasAttribute('reject-request')) {
        ...
        var currency=event.target.getAttribute('currency');
        this.request_id = selected_request_id;
        Swal.fire({
          title: 'Reject Request of '+" " + selected_request_name,
          text:
            'Are you sure? you are rejecting '+" " +currency+ " "+'currency request.'+" "+
            
            'This means the currency will not be created.',
          icon: 'warning',
          ...
          if (result.isConfirmed) {
            ...
            this.approverService.rejectCurrencyRequest(selected_request_id).subscribe(
              (data) => {
                if (data == true) {
                      ...
                      (data) => {
                        if (data === true) {
                          ...
                          this.approverService
                            .rejectCurrencyRequest(selected_request_id)
                            .subscribe(
                              (data) => {
                                if (data == true) {
                                  Swal.fire({
        ...
        Swal.fire({
          ...
          text:
            'Are you sure? you are approving '+" " +currency+ " "+' currency.'+" "+
            'This means the currency will be created.',
          icon: 'warning',
          showCancelButton: true,
          ...
          if (result.isConfirmed) {
            // Swal.fire('Deleted!', 'Your file has been deleted.', 'success');
            this.approverService.approveCurrencyRequest(selected_request_id).subscribe(
              (data) => {
                if (data) {
                  Swal.fire({
                    icon: 'success',
                    ...
                    this.utilService.refreshToken().subscribe(
                      (data) => {
                        if (data === true) {
                          ...
                          this.refresh_token_requested = false;
                          //================================================================================
                          this.approverService.approveCurrencyRequest(selected_request_id)
                            .subscribe(
                              (data) => {
                                if (data) {
                                  Swal.fire({
      ...
      } else if (event.target.hasAttribute('send-currency-remark')) {
        this.router.navigateByUrl(
          'Approver/Currency-requests/currency-remark/' +
            event.target.getAttribute('send-currency-remark')
        );
      }
    });
