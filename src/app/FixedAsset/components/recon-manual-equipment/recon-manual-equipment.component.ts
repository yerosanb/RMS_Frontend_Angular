...
import { ReconManualFixedAssetService } from '../../Abebayehu/service/recon-manual-fixed-asset.service';
declare var window: any;
@Component({
  selector: 'app-recon-manual-equipment',
  templateUrl: './recon-manual-equipment.component.html',
  styleUrls: ['./recon-manual-equipment.component.css']
})
export class ReconManualEquipmentComponent {
  ...
  ngOnInit(): void {
      ...
      this.dtOptions_mms = {
        ...
        ajax: (dataTablesParameters: any, callback: any) => {
          this.reconManualFixedAssetService
            .get_raw_fixed_mms_for_recon_equipment(
    ...
    try {
      this.dtOptions_core = {
        ...
        ajax: (dataTablesParameters: any, callback: any) => {
          this.reconManualFixedAssetService.get_raw_fixed_core_for_recon_equipment().subscribe(
            async (resp: any) => {
              if (resp != null) {
                callback({
                  recordsTotal: resp.recordsTotal,
  ...
  showSelectionEmpty() {
    this.toast.error({
      detail: 'ERROR',
      summary: 'Please select atleast 2 transactions  to  match.',
    });
  }
  IncorrectSelection() {
    this.toast.error({
      ...
      (dtElement: DataTableDirective, index: number) => {
        if (index == 0) {
          ...
          dtElement.dtInstance.then(async (dtInstance: DataTables.Api) => {
            ...
            if (rowData.length > 0) {
              for (let i = 0; i < rowData.length; i++) {
                if (rowData[i].credit != 0) {
                  data_core_amount_sum += rowData[i].credit;
                }
                else {
                  data_core_amount_sum -= rowData[i].debit;
                }
