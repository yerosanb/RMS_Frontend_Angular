...
@Component({
  selector: 'app-view-disposed',
  templateUrl: './view-disposed.component.html',
  styleUrls: ['./view-disposed.component.css']
})
export class ViewDisposedComponent implements OnInit {
  ...
  mms_waiting_fifth_column_title = 'Disposed date';
  ...
  mms_waiting_thirteenth_column_title = 'Asset code';
  mms_waiting_fourteenth_column_title = 'Disposed by';
  ...
  minDateChange(value: Date): void {
    ...
    this.localStorageService.store('min_current_date_disposed', this.initialMinDateValue);
  ...
  maxDateChange(value: Date): void {
    ...
    this.localStorageService.store('max_current_date_disposed', this.initialMaxDateValue);
    if (!this.reload_checker)
      this.dtElements.forEach(
        (dtElement: DataTableDirective, index: number) => {
  ...
  ngOnInit() {
    if (this.localStorageService.retrieve('min_current_date_disposed') == null) {
      this.initialMinDateValue = new Date();
      this.localStorageService.store(
        'min_current_date_disposed',
        this.initialMinDateValue
      );
    } else
      this.initialMinDateValue = new Date(
        this.localStorageService.retrieve('min_current_date_disposed')
      );

    if (this.localStorageService.retrieve('max_current_date_disposed') == null) {
      this.initialMaxDateValue = new Date();
      this.localStorageService.store(
        'max_current_date_disposed',
        this.initialMaxDateValue
      );
    } else
      this.initialMaxDateValue = new Date(
        this.localStorageService.retrieve('max_current_date_disposed')
      );
