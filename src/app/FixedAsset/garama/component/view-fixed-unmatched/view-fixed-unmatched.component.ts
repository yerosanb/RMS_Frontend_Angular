...
@Component({
  selector: 'app-view-fixed-unmatched',
  templateUrl: './view-fixed-unmatched.component.html',
  styleUrls: ['./view-fixed-unmatched.component.css']
})
export class ViewFixedUnmatchedComponent {
  ...
  ngOnInit(): void {
    ...
    try {
      this.dtOptions_mms = {
        ...
        ajax: (dataTablesParameters: any, callback: any) => {
          ...
          .getUnmatchedFixedMMSForView(formatDate(this.initialDateValue, 'yyyy-MM-dd', 'en-US'))
            .subscribe(
              async (resp: any) => {
                console.log(
    ...
    try {
      this.dtOptions_core = {
        ...
        // ajax: '../../../../assets/data/data.json',
        ajax: (dataTablesParameters: any, callback: any) => {
          this.autoReconFixedService.getUnmatchedFixedCoreForView(formatDate(this.initialDateValue, 'yyyy-MM-dd', 'en-US')).subscribe(
            async (resp: any) => {
              if (resp != null) {
                console.log(
                  'response for table: ' + JSON.stringify(resp, null, 2)
